...
export default function LiveTrackingGoogleMap({
  ...
  const mainRouteRef = useRef<any>(null);
  const glowRouteRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (leafletMapRef.current) return; // Prevent duplicate initialization
    ...
    import('leaflet').then(async (L) => {
      ...
      const glowRoute = L.polyline(routePoints, {
        color: '#3B82F6',
        weight: 10,
        opacity: 0.35,
        lineCap: 'round',
      ...
      glowRouteRef.current = glowRoute;
      mainRouteRef.current = mainRoute;
      ...
      houseMarkerRef.current = L.marker([userLat, userLng], { icon: houseIcon }).addTo(map);
        ...
        html: `
          <div style="position: relative; display: flex; flex-direction: column; align-items: center;">
            ...
            <div style="background: #007AFF; color: white; font-size: 9px; font-weight: 800; padding: 3px 9px; border-radius: 12px; margin-top: 4px; white-space: nowrap; box-shadow: 0 2px 8px rgba(0,122,255,0.4); display: flex; align-items: center; gap: 5px;">
              ...
              <span style="opacity: 0.85; font-weight: 600;">• ${(distanceKm || '2.4').replace(/\s*km\s*$/i, '')} km</span>
            </div>
          </div>
        `,
        iconSize: [50, 60],
      ...
      techMarkerRef.current = L.marker([technicianLat, technicianLng], { icon: techIcon }).addTo(map);
  ...
  }, []); // Run ONCE on mount! Zero map blinking!

  // Update Markers & Route smoothly when coordinates change WITHOUT destroying map
  useEffect(() => {
    if (!leafletMapRef.current || !leafRef.current) return;
    const L = leafRef.current;

    // Update Technician Marker Position
    if (techMarkerRef.current) {
      techMarkerRef.current.setLatLng([technicianLat, technicianLng]);
    }

    // Update House Marker Position
    if (houseMarkerRef.current) {
      houseMarkerRef.current.setLatLng([userLat, userLng]);
    }

    // Update Route geometry & OSRM
    const updateRoute = async () => {
      ...
      try {
        ...
        if (data && data.routes && data.routes.length > 0) {
          ...
          if (r.distance) {
            setDistText((r.distance / 1000).toFixed(1) + ' km');
          }
          if (r.duration) {
        ...
        console.warn("OSRM road route update fallback:", err);
      }
      ...
      if (mainRouteRef.current) mainRouteRef.current.setLatLngs(routePoints);
      if (glowRouteRef.current) glowRouteRef.current.setLatLngs(routePoints);
    };

    updateRoute();
  }, [technicianLat, technicianLng, userLat, userLng]);
